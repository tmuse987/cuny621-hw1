...
{
    idx <- mbTrain[i]==0 | is.na(mbTrain[i])  #find index of all rows that are zero
...
```{r}
bat_mod<-lm(TARGET_WINS~TEAM_BATTING_H,mbTrain)
summary(bat_mod)
```
...
```{r}
pitch_mod<-lm(TARGET_WINS~TEAM_PITCHING_H+TEAM_PITCHING_HR,mbTrain)
summary(bat_mod)
plot(mbTrain$TARGET_WINS+mbTrain$TEAM_PITCHING_HR,mbTrain$TEAM_PITCHING_H)
abline(bat_mod)
plot(mbTrain$TARGET_WINS,residuals(bat_mod))
abline(0,0)
