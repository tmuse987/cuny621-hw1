...
We developed models against the "Train" dataset (a random 80% from the entire training csv file), and now let's run against the "test" dataset (being hte other 20%).  We see comparatively similar results, with the model we picked originally (Hits without hr's, BB's, and errors) performing best.
...
When comparing models we can see how they all describe a rather small amount of the variation of the output data TARGET_WINS. Although all model calculated were statistically significat, showing small p-values for the F statistic, the majority of them showed R squared and adjusted R squared of less than 20%. This included models using the calculated variables added to the dataset. A CoxBox transformation was also performed in an attempt to generate a better model, but while its results of around 23% was higher than most models, at least one simpler model was found to have an equal R square of ~23%

Residual plots were performed for all models and in general results were fairly similar. 

## Pick the best regression model

The chosen regresion model was the Batting and Fielding Errors Model. It was the model with the highest acheived R and adjusted R square. It was chosen over the CoxBox transformation model because it is a simpler model. A plot of predicted test data was plotted agaisnt real test data to understand how well the model performs. 
...
```{r}
write.csv(p,"predicted_test_values.csv")
```
